...
def optimize_forecasting_model(
    ...
    print(f"   Function: f(x) where x ∈ ℝ^{num_params}")
    ...
    try:
        ...
        if optimizer.fig_alpha is not None:
            ...
            print(f"   ✓ Alpha plot saved to: {alpha_file}")
        ...
        try:
            ...
            print(f"   ✓ Prediction plot saved to: {pred_file}")
...
Examples:
  ...
  python forecast_example.py --data data/my_data.csv --split 2021-01-01
    ...
    parser.add_argument(
        ...
        type=str,
        default='xgboost',
        choices=['xgboost', 'lightgbm', 'arima', 'ets'],
    ...
    print(f"✓ Best {args.metric.upper()}: {result['metrics'][args.metric]:.4f}")
