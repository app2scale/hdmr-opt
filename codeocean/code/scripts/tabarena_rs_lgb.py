"""
TabArena Random Search Baseline: LightGBM HPO — 16 Datasets
============================================================

Exact baseline counterpart to tabarena_hdmr_lgb.py.
Uses Sobol quasi-random sampling (same as RS-200 in XGBoost benchmark).
...
Env vars:
  ...
  RS_SAMPLES    evaluations per fold (default: 200)
  N_FOLDS       outer CV folds (default: 8)
  SEED          random seed (default: 42)
  LOG_LEVEL     DEBUG | INFO | WARNING (default: INFO)

Usage:
  python tabarena_rs_lgb.py
  DATASETS=46954,46917 N_FOLDS=2 RS_SAMPLES=20 python tabarena_rs_lgb.py  # smoke
...
RS_SAMPLES    : int  = int(os.environ.get("RS_SAMPLES",  "200"))
...
N_DIMS                : int = 12  # TabArena Table C.3 LightGBM

# =============================================================================
# Search Space — TabArena Table C.3 LightGBM (12D)
# =============================================================================

def sample_params(n: int, seed: int) -> List[Dict[str, Any]]:
    """
    Draw n Sobol quasi-random configurations from LightGBM search space.
    Falls back to pseudorandom if scipy unavailable.
    """
    try:
    ...
    for row in raw:
        configs.append({
            ...
            "feature_fraction":   lin_f  (row[1],  0.4,     1.0),
            "bagging_fraction":   lin_f  (row[2],  0.7,     1.0),
            "num_leaves":         log_int(row[3],  2,       200),
            "min_data_in_leaf":   log_int(row[4],  1,        64),
            "extra_trees":        bool(round(float(row[5]))),
            "min_data_per_group": log_int(row[6],  2,       100),
            "cat_l2":             log_f  (row[7],  0.005,   2.0),
            "cat_smooth":         log_f  (row[8],  0.001, 100.0),
            "max_cat_to_onehot":  log_int(row[9],  8,       100),
            "lambda_l1":          lin_f  (row[10], 1e-4,    1.0),
            "lambda_l2":          lin_f  (row[11], 1e-4,    2.0),
            "bagging_freq":       1,
...
def setup_logging(run_id: str) -> logging.Logger:
    ...
    log_file = LOG_DIR / f"rs_lgb_tabarena_{run_id}.log"
    ...
    logger.info("=" * 70)
    logger.info("Random Search (Sobol)  |  LightGBM HPO  |  16 Datasets  v1.0")
...
def evaluate_lgb(params, X_tr, y_tr, X_te, y_te, task, n_classes,
    ...
    try:
        ...
        if is_clf:
            ...
            return {"auc": auc, "accuracy": float(accuracy_score(y_te, preds)),
            ...
            return {"rmse": float(np.sqrt(mean_squared_error(y_te, preds))),
                    "mae":  float(mean_absolute_error(y_te, preds)),
                    "r2":   float(r2_score(y_te, preds))}
    except Exception as exc:
        logging.getLogger("rs.benchmark").error("Test eval failed: %s", exc)
        if is_clf:
            return {"auc": float("nan"), "accuracy": float("nan"), "logloss": float("nan")}
...
# =============================================================================
# Run Random Search on one dataset
# =============================================================================

def run_rs_on_dataset(X: NDArray, y: NDArray, name: str, task: str,
    ...
    logger.info("  │  task=%s  samples=%d/fold  sobol_quasi_random", task, RS_SAMPLES)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splitter.split(X, y), start=1):
        ...
        with tqdm(total=RS_SAMPLES,
            ...
            for cfg in configs:
                lgb_params = {**cfg, "objective": objective, "metric": metric,
                ...
                try:
                    ...
                    if is_clf:
                        ...
                        val_score = float(log_loss(yv, p))
                    else:
                        val_score = float(np.sqrt(mean_squared_error(
                            yv, booster.predict(Xv))))
                except Exception:
                    val_score = 1e6

                eval_log.append(val_score)
                if val_score < best_val:
                    best_val = val_score; best_params = cfg.copy()
                conv_best.append(best_val)
                pbar.update(1)
                pbar.set_postfix({"val": f"{val_score:.4f}", "best": f"{best_val:.4f}"})
        ...
        logger.info("  │  [%s] Fold %d/%d | %s=%.4f | best_val=%.4f | evals=%d | %.1fs | ETA: %s",
    ...
    return {"dataset": name, "task": task, "method": tag,
            "total_evals": RS_SAMPLES, "wall_sec": round(total_sec, 1),
...
def save_results(all_summaries: List[Dict], logger: logging.Logger) -> Path:
    ...
    out_path = TAB_RESULTS / f"rs_lgb_all16_{RUN_ID}.csv"
    ...
    for s in all_summaries:
        for r in s["fold_results"]:
            row: Dict[str, Any] = {
                ...
                "best_val": r["best_val"], "n_evals": r["n_evals"],
...
def print_comparison_report(all_summaries: List[Dict],
    ...
    for task in ("regression", "classification"):
        ...
        logger.info("")
        logger.info("=" * 100)
        logger.info("  %s  ─  RS-200 vs TabArena LightGBM Baselines  |  %s",
                    task.upper(), pm_lbl)
        logger.info("=" * 100)
        logger.info("  %-38s  %-18s  %-16s  %-16s  %-16s",
                    "Dataset", "RS-200", "LGB-Default", "LGB-Tuned", "LGB-Ens")
        logger.info("  " + "─" * 90)
...
def main() -> None:
    ...
    logger.info("  search_space=TabArena C.3 LightGBM (%dD)", N_DIMS)
    ...
    for did in TARGET_DIDS:
        ...
        try:
            ...
            logger.error("RS failed on %s: %s", ds_name, exc)
    ...
    logger.info("Datasets completed : %d / %d",
                len(all_summaries), len(TARGET_DIDS))
