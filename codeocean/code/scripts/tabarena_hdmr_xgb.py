...
# Fixed XGBoost budget (TabArena protocol)
N_ESTIMATORS_FIXED    : int = 10_000
EARLY_STOPPING_ROUNDS : int = 50

# =============================================================================
# Search Space — TabArena Table C.3 (10D, shared for both tasks)
# =============================================================================

PARAM_NAMES: List[str] = [
    "learning_rate",     # 0  LogUniform  [0.005, 0.1]
    "max_depth",         # 1  LogUniformInt [4, 10]
    "min_child_weight",  # 2  LogUniform  [0.001, 5.0]
    "subsample",         # 3  Uniform     [0.6, 1.0]
    "colsample_bylevel", # 4  Uniform     [0.6, 1.0]
    "colsample_bynode",  # 5  Uniform     [0.6, 1.0]
    "reg_alpha",         # 6  Uniform     [1e-4, 5.0]
    "reg_lambda",        # 7  Uniform     [1e-4, 5.0]
    "grow_policy",       # 8  categorical {depthwise, lossguide}
    "max_leaves",        # 9  LogUniformInt [8, 1024]
]
N_DIMS: int = len(PARAM_NAMES)


def decode_params(x: NDArray) -> Dict[str, Any]:
    """Map unit hypercube [0,1]^10 → XGBoost parameter dict (TabArena C.3)."""
    x = np.clip(x, 0.0, 1.0)
    ...
    return {
        ...
        "max_depth":         log_int(x[1], 4,     10),
        "min_child_weight":  log_f  (x[2], 0.001, 5.0),
        "subsample":         lin_f  (x[3], 0.6,   1.0),
        "colsample_bylevel": lin_f  (x[4], 0.6,   1.0),
        "colsample_bynode":  lin_f  (x[5], 0.6,   1.0),
        "reg_alpha":         lin_f  (x[6], 1e-4,  5.0),
        "reg_lambda":        lin_f  (x[7], 1e-4,  5.0),
        "grow_policy":       "lossguide" if x[8] >= 0.5 else "depthwise",
        "max_leaves":        log_int(x[9], 8,     1024),
    }
...
def setup_logging(run_id: str) -> logging.Logger:
    ...
    log_file = LOG_DIR / f"hdmr_tabarena_{run_id}.log"
    ...
    logger.info("=" * 70)
    logger.info("HDMR vs Adaptive HDMR  |  XGBoost HPO  |  All 18 Datasets  v7.0")
...
def load_dataset(did: int, logger: logging.Logger
    ...
    """
    Returns (X, y, name, task).
    task = 'regression' or 'classification'.
    For classification, y is integer-encoded (0..n_classes-1).
    """
    try:
        import ssl
...
def make_regression_objective(
    ...
    """80/20 holdout inner validation, minimizes val RMSE (scaled)."""
    try:
    ...
    def objective_batch(X_batch: NDArray) -> NDArray:
        results = []
        for row in X_batch:
            params = decode_params(row)
            try:
                ...
                val_rmse = float(np.sqrt(mean_squared_error(
                    y_vl, model.predict(X_vl))))
            except Exception as exc:
                _log.debug("Regression eval error: %s", exc)
                val_rmse = 1e6

            eval_log.append((params.copy(), val_rmse))
            if pbar is not None:
                best = min(r for _, r in eval_log)
                pbar.update(1)
                pbar.set_postfix({"val": f"{val_rmse:.4f}", "best": f"{best:.4f}"})
            results.append(val_rmse)
        return np.array(results).reshape(-1, 1)

    return objective_batch, eval_log
...
def make_classification_objective(
    ...
    """
    80/20 holdout inner validation.
    Minimizes val log-loss (lower = better, consistent with HDMR minimization).
    """
    try:
        import xgboost as xgb
    except ImportError:
    ...
    def objective_batch(X_batch: NDArray) -> NDArray:
        ...
        for row in X_batch:
            params = decode_params(row)
            try:
                obj_metric  = "binary:logistic"    if is_binary else "multi:softprob"
                ...
                val_loss = float(log_loss(y_vl, proba))
            except Exception as exc:
                _log.debug("Classification eval error: %s", exc)
                val_loss = 1e6

            eval_log.append((params.copy(), val_loss))
            if pbar is not None:
                best = min(r for _, r in eval_log)
                pbar.update(1)
                pbar.set_postfix({"val_loss": f"{val_loss:.4f}", "best": f"{best:.4f}"})
...
def evaluate_regression(
    best_params: Dict, X_tr_scaled: NDArray, y_tr_scaled: NDArray,
    X_te_scaled: NDArray, y_te_raw: NDArray, sy: StandardScaler,
) -> Dict[str, float]:
    try:
        import xgboost as xgb
        rng_es = np.random.default_rng(SEED)
        n_es   = max(1, int(len(X_tr_scaled) * 0.1))
        idx_es = rng_es.permutation(len(X_tr_scaled))
        model  = xgb.XGBRegressor(
            **best_params,
            n_estimators=N_ESTIMATORS_FIXED,
            early_stopping_rounds=EARLY_STOPPING_ROUNDS,
        ...
        model.fit(
            X_tr_scaled[idx_es[n_es:]], y_tr_scaled[idx_es[n_es:]],
            eval_set=[(X_tr_scaled[idx_es[:n_es]], y_tr_scaled[idx_es[:n_es]])],
            verbose=False,
        )
        preds = sy.inverse_transform(
            model.predict(X_te_scaled).reshape(-1, 1)).ravel()
        return {
...
def evaluate_classification(
    best_params: Dict, X_tr_scaled: NDArray, y_tr: NDArray,
    X_te_scaled: NDArray, y_te: NDArray, n_classes: int,
) -> Dict[str, float]:
    try:
        import xgboost as xgb
        is_binary  = (n_classes == 2)
        obj_metric = "binary:logistic" if is_binary else "multi:softprob"
        eval_metric= "logloss"         if is_binary else "mlogloss"
        xgb_params = {**best_params, "objective": obj_metric,
                      "eval_metric": eval_metric}
        if not is_binary:
            xgb_params["num_class"] = n_classes
        ...
        model.fit(
            X_tr_scaled[idx_es[n_es:]], y_tr[idx_es[n_es:]],
            eval_set=[(X_tr_scaled[idx_es[:n_es]], y_tr[idx_es[:n_es]])],
            verbose=False,
        )
        proba = model.predict_proba(X_te_scaled)
        preds = model.predict(X_te_scaled)
        auc = (float(roc_auc_score(y_te, proba[:, 1])) if is_binary
               else float(roc_auc_score(y_te, proba, multi_class="ovr",
                                        average="macro")))
        return {
            "auc":      auc,
        ...
        return {"auc": float("nan"), "accuracy": float("nan"),
                "logloss": float("nan"), "_error": str(exc)}
...
def run_method_on_dataset(
    ...
    """
    Runs one HDMR variant on a single dataset.
    Returns a summary dict with fold results, metrics, and sensitivity.
    """
    ...
    # Stratified split for classification, regular KFold for regression
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splitter.split(X, y), start=1):
        ...
        # Scale features always; scale target only for regression
        sx    = StandardScaler()
...
def save_results(all_summaries: List[Dict], logger: logging.Logger) -> Path:
    ...
    out_path = TAB_RESULTS / f"hdmr_all18_{RUN_ID}.csv"
...
def print_comparison_report(all_summaries: List[Dict],
    ...
    """Prints a formatted comparison table split by task type."""
    ...
    for task in ("regression", "classification"):
        ...
        # Lower is better for RMSE, higher for AUC
        best_fn = (min if task == "regression" else max)

        logger.info("")
        logger.info("=" * 90)
        logger.info("  %s  ─  %s (mean±std over %d folds)",
        ...
        hdr += "  Winner"
        logger.info(hdr)
        logger.info("  " + "─" * (42 + len(methods) * 22 + 10))
...
def main() -> None:
    ...
    logger.info("  outer_folds=%d  seed=%d  search_space=TabArena C.3 (%dD)",
    ...
    for did in TARGET_DIDS:
        ...
        n_classes = int(np.max(y) + 1) if task == "classification" else 0
